...
// Watch for changes in each brick. 
- (void)addObservers {
...
- (NSUInteger)endingIndexOfBrick:(Brick *)theBrick {
	...
	if (rank == (sortedBricksArray.count - 1)) {
		...
		Brick *nextBrick = [sortedBricksArray objectAtIndex:(rank + 1)];
...
- (NSUInteger)rankOfBrick:(Brick *)theBrick {
	...
	for (int i = 0; i < [sortedBricksArray count]; i++) {
		if (((Brick *)[sortedBricksArray objectAtIndex:i]).startingIndex == theBrick.startingIndex) {
			rank = i;
			break;
		}
	}
